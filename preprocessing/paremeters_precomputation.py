...
def main(scope: str, verbose: bool = False):
    ...
    if dataset_handler.subjects is None or len(dataset_handler.subjects) == 0:
        raise ValueError(f"No subjects found for scope '{scope}' in the dataset.")
    ...
    pathlib.Path('preprocessing/csvs/').mkdir(parents=True, exist_ok=True)  # Ensure directory exists
